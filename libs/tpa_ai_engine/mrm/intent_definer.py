...
class IntentDefiner:
    def __init__(self, policy_manager: PolicyManager, api_key: str):
        ...
        self.intent_spec_prompt_template = load_prompt_from_file('intent_definer_prompt.txt')
        self.clarification_prompt_template = load_prompt_from_file('clarification_prompt.txt')
        self.cache = LLMRedisCache(prefix="llm:intent_spec:")
    ...
    def define_intent_spec_via_llm(self, node: ReasoningNode, application_refs: List[str], application_display_name: str,
        ...
        try:
            ...
            # LLM calls are now direct, with retry logic only
            response_text = ""
            cache_key = f"intent:{node.node_id}:{application_display_name}:{report_type}"
            cached = self.cache.get(cache_key)
            if cached:
                node_provenance.add_action("Cache hit for intent spec", {"cache_key": cache_key})
                return cached
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    print(f"WARN: API attempt {attempt + 1} failed: {type(api_error).__name__} - {str(api_error)[:200]}")
                    if attempt == max_retries - 1:
                        print(f"ERROR: All {max_retries} API attempts failed for clarification intent")
                        raise api_error
                    print(f"Retrying in 10 seconds...")
                    time.sleep(10)  # Wait longer before retry
