...
This frontend is part of a **monorepo architecture** that includes shared backend APIs, data processing pipelines, and common libraries for database models and schemas.

## Monorepo Integration

This frontend application integrates with the TPA-v2 monorepo system:

- **Backend API**: Located in `../backend/` providing RESTful services
- **Data Pipeline**: Located in `../data_pipeline/` for document processing
- **Shared Models**: Database and API schemas are centralized in `../../libs/`
- **Common Configuration**: Root-level Docker Compose and environment setup

The frontend communicates with backend APIs that use shared database models and schemas, ensuring consistency across the entire system.
...
```sh
# From the monorepo root, ensure backend is set up first
...
- **Mock Data:**  
  The app uses mock data/services for demonstration. See [`src/lib/services/mockData.ts`](src/lib/services/mockData.ts). In production, this connects to the backend API in `../backend/`.
